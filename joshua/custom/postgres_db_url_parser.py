...
def database_url_postgres_parser_with_port(database_url: str):
    ...
    return dict(user=user, password=password, host=host, port=port,
                name=db_name)  # return mapped dictionary with values and keys for relevant elements


# postgres://USER:PASSWORD@HOST:PORT/NAME

def database_url_postgres_parser_no_port(database_url: str):
    """ Collect all necessary information from PostgreSQL database URL.
    ...
    db_name, host = parser_core(db_url, '/')  # extract trimmed name value and port value
